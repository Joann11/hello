...
def create_app():
    ...
    # Initialize Flask-BabelEx
        ...
        def unauthorized_callback():            # In call back url we can specify where we want to 
            return redirect(url_for('login')) # redirect the user to login page
    ...
    # Define the User data-model.
        ...
        class Post(db.Model):
            ...
            postscore = db.Column(db.Integer)
        ...
        with app.app_context():
            ...
            if not Role.query.filter_by(name='Professional').one():
                Role(name='Professional')
                db.session.commit()

            if not Role.query.filter_by(name='User').one():
                Role(name='User')
            ...
            #This is to create an Admin page with custom index view
            #Code Copied from https://flask-admin.readthedocs.io/en/latest/, 15/03/2023
        ...
        class PostForm(FlaskForm):
            post = StringField(validators=[
                                DataRequired(), Length(min=6, max=200000)], render_kw={"placeholder": "Thoughts..."})
        ...
        def home():  
            posts = Post.query.filter(Post.date ==  date.today()).filter(Post.postscore < 0).order_by(Post.postscore )
            users = User.query.filter(User.score < 0).order_by(User.score)

            print(posts)

            return render_template('home.html', adminuser = current_user, users = users, posts = posts, date = date.today())
        ...
        def diarypage():
            ...
            if request.method == "POST":
                ...
                if 'post' in request.form:
                        ...
                        total, comment = calculatescore_function(text)     
                        print("text:", text, total)

                        postNew = Post(text= text, date=datetime.today().date(), time=datetime.now().time(), postscore = total, author= current_user)
                        #update userscore
                        setattr(current_user, 'score', current_user.score + total)
       ...
       # user_manager = UserManager(app, db, User)
        ...
        def profilebackend(username):
            ...
            return render_template('profilebackend.html', posts = posts, user = user, username = username)
        ...
        def postanalyze(username):
                  if request.method == "POST":
                    ...
                    textP = input_text
                    calculatescore_function(getText())
                    ...
                    data = {
                        ...
                        'graph': graphP,
                        'url':  url_for('analyze', username=username, postid = postid, _external=True)
        ...
        @app.route('/evaluation/<username>/<postid>')
        def analyze(username, postid):
                    ...
                    #input_text = request.form['post_id']
                    f = open("example.txt", "w")
                    f.write(input_text)
                    # f.write(t)
                    f.close()
                    textP = nlp(input_text)
                    total, comment = calculatescore_function(textP)
                    sentences = list(textP.sents)
                    keywords = list(detectedwords.keys())
                    adverblist = detect.detect_adverb(textP)
                    adverbs = [ad[3] for ad in adverblist]
                    negativedictionary = negativedic
                    posdictionary = positivedic
                    neutraldictionary = neutraldic
                    print(adverbs)

                    
                    c = plotGraph.sentenceAnalysis(textP)
                    if c:
                        c = [0] + c
                    graphP = plotGraph.plotGraphC(c)
                    
                    return render_template('textanalysis.html', textP = str(textP), total = total, comment = comment, sentences = sentences, adverbs = adverbs, keywords = keywords, neutraldictionary = neutraldictionary, negativedictionary = negativedictionary, posdictionary = posdictionary, graphP = graphP)
